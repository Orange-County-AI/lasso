...
// Claude Code's ~/.claude/themes/herdr.json, and lasso's settings.json
// (.theme.resolved, read by claude-contextline) — so agents render in step
// with herdr. Writes go through the Backend interface, so the active remote
// host gets the same treatment over SFTP (see syncRemoteTheme).
//
