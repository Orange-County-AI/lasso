...
import { getMode, setLuvusAppearance } from "@/lib/mode"

// The terminal iframes whose xterm.js theme we keep in sync with Luvus's: the
// left Luvus terminal and the right shell.
...
const TERM_FONT_STYLE_ID = "luvus-term-font"
...
// background between Luvus's right edge and the splitter, which reads as a gap
...
const TERM_FIT_STYLE_ID = "luvus-term-fit"
const TERM_FIT_CSS = [
...
export function applyTermFont(tries = 0) {
  ...
  for (const el of termFrames()) {
    try {
      ...
      if (doc.__luvusFontWired) continue
      doc.__luvusFontWired = true
...
// the next Luvus theme change or a full page reload, while the React/CSS side
...
// The <style> that overrides the chrome's --h-* vars with Luvus's palette while
// the user is in "luvus" appearance mode (see lib/mode.ts). Appended after the
...
const LUVUS_CHROME_STYLE_ID = "lasso-luvus-chrome"

// applyLuvusChrome paints the chrome from Luvus's resolved palette, reproducing
// the pre-Nothing behavior where the whole UI tracked Luvus's theme. `css` is
// /api/theme's bare "--bg: …;" declaration block; prefix each property to
// "--h-" to match the token names index.css cascades from.
// Idempotent — reuses the style node.
function applyLuvusChrome(css: string) {
  let el = document.getElementById(
    LUVUS_CHROME_STYLE_ID
  ) as HTMLStyleElement | null
  if (!el) {
    el = document.createElement("style")
    el.id = LUVUS_CHROME_STYLE_ID
    document.head.appendChild(el)
  }
  el.textContent = `:root{${css.replaceAll("--", "--h-")}}`
}

// clearLuvusChrome removes the override so the chrome falls back to the static
// Nothing palette (used by every non-"luvus" appearance mode).
function clearLuvusChrome() {
  document.getElementById(LUVUS_CHROME_STYLE_ID)?.remove()
}

// refreshTheme pulls the ACTIVE Luvus theme and applies it. Luvus is the source
// of truth for it: lasso reads the palette and never writes one back. The
// terminals always track it — it sets the xterm.js palette inside the
...
// come from index.css and follow the light/dark mode) — EXCEPT in "luvus"
// appearance mode, where we also repaint the chrome from Luvus's palette so the
// whole UI matches the terminal, and record Luvus's own light/dark verdict for
// it so the html class matches what the override paints.
export async function refreshTheme() {
  ...
  applyTermFont(0)
  setLuvusAppearance(t.appearance)
  if (getMode() === "luvus") applyLuvusChrome(t.css)
  else clearLuvusChrome()
}
