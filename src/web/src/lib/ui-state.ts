...
// Persisted, SQLite-backed UI preferences (sidebar layout and Files-tab click
// behavior). One shared React Query cache is the source of truth in this tab;
// the server merges partial patches so concurrent tabs cannot clobber fields
// they did not touch, then bumps ui_state_rev over SSE so every open tab
// converges on the same state (see syncUIState).
