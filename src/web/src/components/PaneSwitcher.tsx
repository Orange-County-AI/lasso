...
// PaneSwitcher: the mobile picker over the tab's host's panes. Type to filter;
// ↑/↓ to move; Enter to focus the chosen pane in the herdr terminal (handing the
// keyboard straight to its xterm).
//
// Nothing opens it today — ⌘K and the dial's Search command both go to herdr's
// OWN pane search (openHerdrGoto), a software keyboard having no prefix key to
// chord ⌘K with — so it is mounted by App.tsx with `open` permanently false.
// Its other distinction is untouched: it is the only picker that can show CLOSED
// sessions (the Active filter off) and reopen their workspaces.
//
// It deliberately drops the other hosts' rows /api/all-panes also carries:
// picking one of those switches lasso's active host — a seconds-long terminal
// reload — from a list this tab's terminal is not showing.
