...
type createAgentIn struct {
	...
	Model        string `json:"model,omitempty" jsonschema:"Model for the agent's CLI (passed to its --model flag), e.g. \"opus\", \"sonnet\", \"haiku\" for claude, \"gpt-5.1-codex\" for codex, or \"anthropic/claude-sonnet-4-5\" for opencode (provider/model). omp and pi take a pattern matched against the authenticated provider catalogs, so a bare \"opus\" or \"gpt-5.5\" resolves as well as an exact \"provider/id\". Omit for the harness default."`
