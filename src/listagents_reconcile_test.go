...
// listBackend is one fake host's runtime for the list_agents path: the pane
// enumeration answers from a fixture through the real UHP wire shapes, or fails
// wholesale when down is set (server restarting, host briefly unreachable).
...
func (b *listBackend) LuvusCall(method string, params any) (json.RawMessage, error) {
	if b.down {
		return nil, &luvusError{Code: "internal", Message: "luvus is restarting"}
	}
...
// The headline case: list_agents returns only agents luvus still has a pane for,
// and it reconciles the rest away as it goes — while the foreign session
...
func TestListAgentsReconcilesAndKeepsForeignSessions(t *testing.T) {
	...
	if err := appendAgent("local", reapRec("livesone", "1")); err != nil {
		t.Fatal(err)
	}
	if err := appendAgent("local", reapRec("deadone", "2")); err != nil {
		t.Fatal(err)
	}
	b := &listBackend{memBackend: newMemBackend(), panes: []pane{
		{PaneID: "1", WorkspaceID: "w1", TabID: "w1-t1", Agent: "claude", AgentStatus: "idle"},
		// A pane lasso did not create: a bot in its own workspace. Its only
		// human-facing name is that workspace's name — UHP publishes no pane
		// title to fall back to.
		{PaneID: "5", WorkspaceID: "w5", TabID: "w5-t1", WorkspaceLabel: "Clem (OCAI)",
	...
	if out.LuvusError != "" {
		t.Fatalf("unexpected luvus_error: %s", out.LuvusError)
	}
	...
	if _, ok := got["deadone"]; ok {
		t.Error("agent whose pane luvus no longer has is still listed")
	}
	foreign, ok := got["5"]
	if !ok {
		t.Fatal("foreign luvus session was dropped — reconciliation must not touch panes lasso did not create")
	}
...
// A luvus that cannot be enumerated is not evidence that anything died: the
// listing says so (luvus_error, i.e. PARTIAL) and every record survives, however
// many times the call is repeated.
func TestListAgentsDoesNotReapWhenLuvusIsDown(t *testing.T) {
	openTestDB(t)
	resetAgentReapMiss(t)
	if err := appendAgent("local", reapRec("alive", "1")); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < agentReapMisses+2; i++ {
		...
		if out.LuvusError == "" {
			t.Fatal("a failed luvus enumeration did not report luvus_error")
		}
	...
	if !liveIDs(t, "local")["alive"] {
		t.Error("an unreachable luvus reaped a record")
	}
}
...
func TestListAgentsIDsAllResolve(t *testing.T) {
	openTestDB(t)
	resetAgentReapMiss(t)
	for i, pane := range []string{"1", "2", "w3:p1"} {
		if err := appendAgent("local", reapRec(fmt.Sprintf("a%d", i), pane)); err != nil {
			t.Fatal(err)
		}
	}
	b := &listBackend{memBackend: newMemBackend(), panes: []pane{
		{PaneID: "2", WorkspaceID: "w2", Agent: "claude", AgentStatus: "idle"},
	}}
	stubListBackend(t, b)
	...
	panes, err := hostLuvusPanesErr(b, "local")
	if err != nil {
		t.Fatal(err)
	}
	for _, a := range out.Agents {
		...
		if _, ok := findPane(panes, tgt.PaneID); !ok {
			t.Errorf("listed agent %q resolves to pane %q that luvus does not have", a.ID, tgt.PaneID)
		}
	}
}
