...
- **Every sidebar endpoint takes a host.** `/api/files`, `/api/file`, `/api/diff`, `/api/diff-file` (`?host=`), `/api/file-write`, `/api/file-delete`, `/api/file-rename` (a `host` body field) and `/api/file-upload` (a `host` form field) all resolve through `namedHostBackend` (empty = the active host, `gridHostAllowed` gate, 502 `host "x" not available`), and tilde-expand against THAT backend. The diff helpers take a `Backend` for the same reason — there is no active-backend `gitOut` any more. Half a cutover here means an editor saving to the wrong machine.
